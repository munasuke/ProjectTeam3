...
FragmentCommonData FragmentSetupSimple(VertexOutputBaseSimple i, float4 dissolveUV, float3 dissolveVPos)
{
	//VacuumShaders
	half alpha = ReadDissolveAlpha(i.tex.xy, dissolveUV, dissolveVPos);
	DoDissolveClip(alpha);

	#ifdef _MAINTEXCUTOFF_ON
				clip(tex2D(_MainTex, i.tex.xy).a * _Color.a - _Cutoff);
		#endif
...
FragmentCommonData FragmentSetupSimpleAdd(VertexOutputForwardAddSimple i, float4 dissolveUV, float3 dissolveVPos)
{
	//VacuumShaders
	half alpha = ReadDissolveAlpha(i.tex.xy, dissolveUV, dissolveVPos);
	DoDissolveClip(alpha);
