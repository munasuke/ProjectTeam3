...
#include "Assets/VacuumShaders/Advanced Dissolve/Shaders/cginc/AdvancedDissolve.cginc"
		...
		struct pixel_t {
			...
			fixed4	color			: COLOR;
			float4	atlas			: TEXCOORD0;		// Atlas (x, y), worldPos (z, w)
			float4	param			: TEXCOORD1;		// alphaClip, scale, bias, weight
			float4	mask			: TEXCOORD2;		// Position in object space(xy), pixel Size(zw)
			float4	viewDir			: TEXCOORD3;		// view (x, y, z), worldPos (w)
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			pixel_t output = {
				vPosition,
				input.color,
				float4(input.texcoord0.xy, advancedDissolve_worldPos.xy),
				float4(alphaClip, scale, bias, weight),
				half4(vert.xy * 2 - clampedRect.xy - clampedRect.zw, 0.25 / (0.25 * half2(_MaskSoftnessX, _MaskSoftnessY) + pixelSize.xy)),
				float4(mul((float3x3)_EnvMatrix, _WorldSpaceCameraPos.xyz - mul(unity_ObjectToWorld, vert).xyz), advancedDissolve_worldPos.z),
			#if (UNDERLAY_ON || UNDERLAY_INNER)
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
			...
			float3 worldPos = float3(input.atlas.zw, input.viewDir.w);
#ifdef _DISSOLVETRIPLANAR_ON
			float alpha = ReadDissolveAlpha_Triplanar(input.coords, input.objNormal, worldPos);
#else
			float alpha = ReadDissolveAlpha(input.atlas.xy, input.dissolveUV, worldPos);
#endif				
			DoDissolveClip(alpha);



			float c = tex2D(_MainTex, input.atlas.xy).a;
			...
			float3 n = GetSurfaceNormal(input.atlas.xy, weight, dxy);
			...
			fixed4 reflcol = texCUBE(_Cube, reflect(input.viewDir.xyz, -n));
		...
		//#endif
			...
			faceColor.rgb += DoDissolveEmission(alpha, 0);
